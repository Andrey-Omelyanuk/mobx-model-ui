...
describe('DateDescriptor', () => {
    ...
    describe('toString', () => {
        it('10/10/2010 => "10.10.2010"' , () => { expect(desc.toString(new Date(Date.UTC(2010, 9, 10)))).toBe('2010-10-10T00:00:00.000Z') })
    ...
    describe('validate', () => {
        it('min', async () => {
            const desc = new DateDescriptor({min: new Date(Date.UTC(2020, 0, 1))})
            expect(() => desc.validate(new Date(Date.UTC(2019, 11, 31)))).toThrow('Date should be later than 2020-01-01T00:00:00.000Z')
            expect(() => desc.validate(new Date(Date.UTC(2021, 11, 31)))).not.toThrow()
        })
        it('max', async () => {
            const desc = new DateDescriptor({max: new Date(Date.UTC(2020, 0, 1))})
            expect(() => desc.validate(new Date(Date.UTC(2019, 11, 31)))).not.toThrow()
            expect(() => desc.validate(new Date(Date.UTC(2021, 11, 31)))).toThrow('Date should be earlier than 2020-01-01T00:00:00.000Z')
        })
    })
})
