...
- **Variable Pattern**: any observable value (filter, query parameter, form field) is a Variable<T>. Supports debounce, disabled, isNeedToUpdate, errors, validation, synchronization (localStorage → cookie → URL in initialization priority order). Tracks dirty state via `initialValue` / `isDirty` / `markClean()` / `reset()`.
- **Form Lifecycle**: `submit()` → validation (`isReady`) → `apply()` → `markClean()` → `onSuccess()` || `errorHandler()` maps errors (`err.response.data` separating field errors / `non_field_errors` / unknown). `cancel()` calls an external callback. Form-level dirty state aggregates from all inputs via `isDirty` and is automatically reset on successful submit.
...
- **Variable `initialValue` is set after sync handlers**: `initialValue` captures the final value at the end of the constructor, after all sync handlers have run. This ensures the dirty state baseline reflects the synced value, not the raw constructor default.
- **Form dirty state**: `Form.isDirty` aggregates `Variable.isDirty` from all inputs. After a successful `submit()` (when `apply()` resolves without error), `markClean()` is called automatically so the form is clean again. On failed submit, the dirty state persists.
